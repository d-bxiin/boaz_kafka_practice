...
기본적인 스트리밍 파이프라인 구조를 직접 실행해보는 것이다.
...
```
[계좌 거래 Producer] -----> account-events ─┐
                                             ├──> stream-processor ──> risk-check-events
[카드 결제 Producer] -----> card-events   ──┘
                                             └──> transaction-dlq
```
...
`stream_processor.py`에 위험도 판단 로직을 추가해서, `risk_level` 필드를 `low` / `medium` / `high` 3단계로 붙여보는 것이 목표이다.

### 📌 과제 요구 사항

**`normalized` 딕셔너리에 `risk_level` 필드 추가하기 (3단계)**

아래 기준으로 `risk_level`을 계산해서, `normalized`를 만드는 곳에 추가한다. (계좌 거래용, 카드 결제용 **두 군데 모두**에 똑같이 추가한다.)

| amount 기준 | risk_level |
|---|---|
| 100만원 이상 | `"high"` |
| 30만원 이상 ~ 100만원 미만 | `"medium"` |
| 30만원 미만 | `"low"` |

<details>
<summary>힌트</summary>

`stream_processor.py`에서 `normalized = {...}` 부분을 찾는다.

```python
if data["amount"] >= 1_000_000:
    risk_level = "high"
elif data["amount"] >= 300_000:
    risk_level = "medium"
else:
    risk_level = "low"
...
`account-events`용 블록, `card-events`용 블록 **둘 다** 똑같이 추가해야 한다.

</details>

**다른 건 건드리지 않기**

- 메시지 구조를 바꾸지 않는다 (필드 하나만 추가)
- `consumer_risk.py`, DLQ 로직은 그대로 둔다

### ✅ 성공 조건

- `stream_processor.py`가 에러 없이 실행된다
- "변환된 데이터" 로그에 `risk_level` 필드(`"low"` / `"medium"` / `"high"`)가 보인다

### 📸 제출물

1. **4개 터미널이 정상 동작하는 화면 캡처** (Producer 2개 + 스트림 프로세서 + Consumer, 전체 파이프라인이 끝까지 잘 돌아가는지)
2. `stream_processor.py` 실행 화면 캡처 ("변환된 데이터"에 `risk_level` 필드가 `low`/`medium`/`high` 3단계로 보이도록)

---

## 💡 확장 아이디어 (선택, 시간 남으면)

- 같은 `user_id`가 짧은 시간 안에 여러 번 거래하면 `⚠️ 다중 거래 의심`으로 표시해보기
